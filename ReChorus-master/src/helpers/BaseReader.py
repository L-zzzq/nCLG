...
class BaseReader(object):
    ...
    def _read_data(self):
        ...
        self.data_df = dict()
        for key in ['train', 'dev', 'test']:
            self.data_df[key] = pd.read_csv(normalize_path(os.path.join('..',self.prefix, self.dataset, key + '.csv')), sep=self.sep).reset_index(drop=True).sort_values(by = ['user_id','time'])

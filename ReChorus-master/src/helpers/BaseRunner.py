...
class BaseRunner(object):
	...
	def train(self, data_dict: Dict[str, BaseModel.Dataset]):
		...
		# print("in runner",model.encoder.embedding_dict['user_emb'])
	...
	def fit(self, dataset: BaseModel.Dataset, epoch=-1) -> float:
		model = dataset.model
		# print("in fit",model.encoder.embedding_dict['user_emb'])
		if model.optimizer is None:
			model.optimizer = self._build_optimizer(model)
		dataset.actions_before_epoch()  # must sample before multi thread star
		model.train()
		loss_lst = list()
		align_list=list()
		comp_list=list()
		...
		for batch in tqdm(dl, leave=False, desc='Epoch {:<3}'.format(epoch), ncols=100, mininterval=1):		
			...
			# loss,align,comp = model.loss(out_dict)
			loss.backward()
			model.optimizer.step()
			# align_list.append(align)
			# comp_list.append(comp)
			loss_lst.append(loss.detach().cpu().data.numpy())
		# logging.info('Epoch {:<5} loss={:<.4f}  align={:<.4f} comp={:<.4f})'.format(epoch + 1, np.mean(loss_lst).item(),np.mean(align_list).item(),np.mean(comp_list).item()))

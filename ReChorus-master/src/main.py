...
def main():
	...
	# Read data
	corpus_path = normalize_path(os.path.join('..',args.path, args.dataset, model_name.reader+args.data_appendix+ '.pkl'))
	...
	for phase in ['train', 'dev', 'test']:
		...
		# print("encoder",data_dict[phase].model.encoder.embedding_dict['user_emb'])
	...
	# # 奇异值分析代码
	# with torch.no_grad():
	# 	# LightGCN模型中的接口获取全量 embedding.
	# 	user_emb, item_emb = model.encoder.get_all_embeddings() 
	
	# # 定义计算奇异值谱的函数	
	# def plot_singular_values(embeddings, title='User Embedding Spectrum'):
	...
	# #执行计算并绘图
	# user_s = plot_singular_values(user_emb, "User")
	# item_s = plot_singular_values(item_emb, "Item")

	# plt.figure(figsize=(8, 5))
	# plt.plot(user_s, label=init_args.model_name, color='blue')
	# plt.plot(item_s, label=init_args.model_name, color='red')
	# plt.title("Singular Value Spectrum")
	# plt.xlabel("Singular Value Rank Index")
	# plt.ylabel("Singular Values (Normalized)")
	# plt.legend()
	# plt.grid(True, linestyle='--', alpha=0.5)
	# plt.show()

	# logging.info("Singular value distribution plotted.")
